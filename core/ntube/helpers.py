"""Various helper functions implemented by ntube."""
import functools
import gzip
import json
import logging
...
from core.ntube.exceptions import RegexMatchError
...
class DeferredGeneratorList:
    ...
    ntube has some continuation generators that create web calls, which means
...
def setup_logger(level: int = logging.ERROR, log_filename: Optional[str] = None) -> None:
    ...
    logger = logging.getLogger("ntube")
...
def create_mock_html_json(vid_id) -> Dict[str, Any]:
    ...
    gzip_filepath = os.path.join(ntube_mocks_path, gzip_filename)
