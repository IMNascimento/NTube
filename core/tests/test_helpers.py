...
from ntube import helpers
from ntube.exceptions import RegexMatchError
from ntube.helpers import cache, create_mock_html_json, deprecated, setup_logger
from ntube.helpers import target_directory, uniqueify
...
@mock.patch("ntube.helpers.logging")
...
def test_create_mock_html_json(mock_url_open, mock_open):
    ...
    # Get the ntube directory in order to navigate to /tests/mocks
    ntube_dir_path = os.path.abspath(
        os.path.join(
            os.path.dirname(__file__),
            os.path.pardir
        )
    )
    ntube_mocks_path = os.path.join(ntube_dir_path, 'tests', 'mocks')
    gzip_html_filepath = os.path.join(ntube_mocks_path, gzip_html_filename)
